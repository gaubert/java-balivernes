...
=head2 execute
  ...
  extract all the necessary data to create a sample environement for sampml
...
sub execute
{
  ...
  my $file = "$SQLDIR/create_schema.sql";

  my $commandToRun = "sqlplus $USER/$PASS\@$DB < $file > $file.log";
...
insert_sample_data - insert in the database all the necessary data to have a working test sample for sampml
