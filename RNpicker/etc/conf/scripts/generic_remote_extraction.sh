...
if (\$total == 0)
{
   #total is null so ignore it and transfer until eof
   while ( ( (\$n = read(\$fhandle,\$buffer,\$blocksize)) > 0 ) )
   {
      # print buffer
      print "\$buffer";
   }
...
{
    #total is > 0 so do not ignore it and transfer until total reached
