...
def parse_arguments(a_args):
    ...
    try:
        (opts,_) = getopt.getopt(a_args, "hs:f:e:d:v", ["help", "s=","from=","end=","dir=","version"])
...
class Runner(object):
    ...
    def __init__(self):
        ...
        #setup conf
        os.environ['SAMPML_CONF_DIR'] = get_tests_dir_path()
        
        os.environ[Conf.ENVNAME] = '%s/%s'%(get_tests_dir_path(),'rnpicker.config')
