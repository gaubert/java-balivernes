...
class SaunaRenderer(BaseRenderer):
    ...
    def _getROIInfo(self, id):
        ...
        for roi in rois:
            ...
            dummy_template = re.sub("\${DETNETCOUNTS}", "%s %s" % (str(roi[u'DET_BKGND_COUNT']), str(roi[u'DET_BKGND_COUNT_ERR'])), dummy_template)
            dummy_template = re.sub("\${GASNETCOUNTS}", "%s %s" % (str(roi[u'GAS_BKGND_COUNT']), str(roi[u'GAS_BKGND_COUNT_ERR'])), dummy_template)
