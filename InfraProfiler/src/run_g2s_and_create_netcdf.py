...
from utils import CLIError, FindError, ConfAccessError, ffind, makedirs, ftimer
import const
...
def get_noaa_indices(a_ecmwf_file_name):
    ...
    the_date = get_datetime_from_ecmwf_file_name(a_ecmwf_file_name)
...
def call_g2s_client(a_netcdf_dir, a_file_path, a_ecmwf_path, a_lats, a_lons, a_sta_names):
    ...
    err = create_netcdf('%s/G2SECMWF_%s_STA.nc' % (a_netcdf_dir,dt.strftime('%y%m%d%H') ), lat_points, lon_points, nz, cz, uz, vz, dt, a_sta_names)
...
def create_netcdf(a_netcdf_filename, a_lat_points,a_lon_points, a_nb_levels, a_celerity_arr, a_u_arr, a_v_arr, a_time,a_loc_names):
    ...
    #create file
    dataset = rootgrp = Dataset(a_netcdf_filename, 'w', format=netcdf_format)
